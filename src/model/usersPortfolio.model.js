...
const usersPortfolioModel = {
    ...
    read: function (search, sortBy = 'ASC', limit = 25, offset = 0) {
        ...
        return new Promise((resolve, reject) => {
            db.query(
                ...
                FROM hirejob_portfolio p
                INNER JOIN hirejob_portfolio_images pi ON p.id_portfolio = pi.id_portfolio
    ...
    update: ({ id_portfolio, project_name, link_repository, img_portfolio }) => {
        ...
        return new Promise((success, failed) => {
            db.query(`SELECT * FROM hirejob_portfolio WHERE id_portfolio='${id_portfolio}'`, (error, dataRes) => {
                if (error) {
                    ...
                    if (dataRes.rows.length == 0) {
                        ...
                        db.query(`UPDATE hirejob_portfolio SET project_name='${project_name || dataRes.rows[0].project_name}', link_repository='${link_repository || dataRes.rows[0].link_repository}' WHERE id_portfolio='${id_portfolio}'`, (error) => {
                            if (error) {
                                return failed(error.message)
                            } else {
                                if (img_portfolio.length == 0) {
                                    db.query(`UPDATE hirejob_portfolio_images SET project_name=$1 WHERE id_portfolio=$2`, [project_name || dataRes.rows[0].project_name, id_portfolio], (err) => {
                                        if (err) {
                                ...
                                db.query(`SELECT id_img,filename FROM hirejob_portfolio_images WHERE id_portfolio='${id_portfolio}'`, (errOld, resultOld) => {
                                    ...
                                    for (let i = 0; i < img_portfolio.length; i++) {
                                        if (i >= resultOld.rows.length) {
                                            // console.log(`file ${i}`);
                                            db.query(`INSERT INTO hirejob_portfolio_images (id_img, id_portfolio, project_name, filename) VALUES ($1, $2, $3, $4)`, [uuidv4(), id_portfolio, project_name || dataRes.rows[0].project_name, img_portfolio[i].filename], (err) => {
                                                if (err) return failed(err.message)
                                            })
                                        } else {
                                            // console.log(`file ${i}`);
                                            db.query(`UPDATE hirejob_portfolio_images SET project_name=$1, filename=$2 WHERE id_img=$3`, [project_name || dataRes.rows[0].project_name, img_portfolio[i].filename, resultOld.rows[i].id_img], (err) => {
                                                if (err) return failed(err.message)
    ...
    remove: (id) => {
        return new Promise((resolve, reject) => {
            db.query(
                `DELETE from hirejob_portfolio WHERE id_portfolio='${id}'`,
                (err, result) => {
                    if (err) {
                        return reject(err.message)
                    } else {
                        db.query(`DELETE FROM hirejob_portfolio_images WHERE id_portfolio='${id}' RETURNING filename`, (err, result) => {
