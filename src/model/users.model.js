...
const usersModel = {
    ...
    create: ({ full_name, img_profile, email, password, phone, job_desk, employment_type, domicile, ig_account, github_account, gitlab_account, description }) => {
        return new Promise((resolve, reject) => {
            db.query(
                `INSERT INTO hirejob_users (id_user, full_name, img_profile, email, password, phone, job_desk, employment_type, domicile, ig_account, github_account, gitlab_account, description) VALUES ('${uuidv4()}','${full_name}','${img_profile}','${email}','${password}','${phone}','${job_desk}','${employment_type}','${domicile}', '${ig_account}', '${github_account}', '${gitlab_account}', '${description}')`,
                (err, result) => {
                    if (err) {
    ...
    read: function (search, employment_type, sortBy = 'ASC', limit = 25, offset = 0) {
        return new Promise((resolve, reject) => {
            db.query(
                `SELECT * from hirejob_users ${this.query(search, employment_type, sortBy, limit, offset)}`,
                (err, result) => {
                    // console.log(result);
                    if (err) {
                        return reject(err.message)
    ...
    readDetail: (id_user) => {
        return new Promise((resolve, reject) => {
            db.query(
                `SELECT * from hirejob_users WHERE id_user='${id_user}'`,
                (err, result) => {
                    if (err) {
                        return reject(err.message)
                    } else {
    ...
    update: function (req, id) {
        return new Promise((success, failed) => {
            ...
            db.query(`SELECT * FROM hirejob_users WHERE id_user='${id}'`, (error, result) => {
                if (error) {
    ...
    remove: (id_user) => {
        return new Promise((resolve, reject) => {
            db.query(
                `DELETE from hirejob_users WHERE id_user='${id_user}' RETURNING img_profile`,
                (err, result) => {
                    // console.log(result);
                    if (err) {
                        return reject(err.message)
